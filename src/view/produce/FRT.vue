    ...
    <div ref="produce" class="data-show" v-show="isShowPage">
      <div class="content">
        <div class="content-list">
          <div v-for="item in scratchData.scratch_info.list" :class="[{'bonus': ((win == item.data) && isShow)}]">
            <img :src="'../../../static/images/scratch_fruit/'+item.data+'.png'" alt="">
            <img :src="'../../../static/images/scratch_fruit/prize_'+item.money+'.png'" alt="">
          </div>
        </div>
      </div>
    </div>
    <canvas id="frt" :class="[{'on': payout}]">您的浏览器不支持canvas技术，请更换浏览器</canvas>
  ...
  export default {
    ...
    data() {
      return{
        ...
        img: {
          canvasImg: "http://scard-images.mohasomnang.com/tickets/2019/FRT/FRT-pre.jpg"
        },
    ...
    methods: {
      findWinNum() {
        let arr = [];
        this.scratchData.scratch_info.list.forEach((val, idx) => {
          arr.push(val.data);
        })
        this.win = this.arrCount(arr);
      },
      arrCount(arr) {
...
<style lang="less" scoped>
  .frt{
    ...
    .data-show{
      ...
      padding-left: 15px;
      padding-right: 15px;
      background: url(http://scard-images.mohasomnang.com/tickets/2019/FRT/FRT-open.jpg);
  ...
  .content{
    ...
    .content-list{
      ...
      div{
        ...
        &:nth-of-type(1) {
          transform: translate3d(-52.5px, -160px, 0);
        }
        &:nth-of-type(2) {
          transform: translate3d(-125px, -90px, 0);
        }
        &:nth-of-type(3) {
          transform: translate3d(15px, -90px, 0);
        }
        &:nth-of-type(4) {
          transform: translate3d(-125px, -15.75px, 0);
        }
        &:nth-of-type(5) {
          transform: translate3d(15px, -15.75px, 0);
        }
        &:nth-of-type(6) {
          transform: translate3d(-52.5px, 55px, 0);
        }
      }
    }
  }
